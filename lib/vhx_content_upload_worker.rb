...
class VhxContentUploadWorker
  ...
  def source_location(vimeo_video)
    ...
    source_files = []
    source_files += Array(vimeo_video['files']&.select { |link| link['quality'] == 'source' || link['width'].to_i >= 1280 })
    source_files += Array(vimeo_video['download']&.select { |link| link['quality'] == 'source' || link['width'].to_i >= 1280 })
